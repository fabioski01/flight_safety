"""
This Python script is designed to processe state vectors stored in a CSV file, 
transforming them into a KML format for visualization in mapping applications.

Key features include:
- Generation of KML files that include trajectory paths, impact points, and separation points for 
various events.
- Handling of atmospheric corrections and adjustments for geographical coordinates based on launch pad
locations (to be implemented).

The script uses several libraries, including lxml for XML handling, and pykml for KML file generation. 
Functions defined within the script include:
- `save_kml_output`: Generates and saves KML output for trajectory visualization.
- `read_and_convert_visibility`: Reads state vectors from a CSV file, processes them, and invokes KML output generation with visibility circles.

The script is intended for use in launch vehicle simulations, particularly to visualize trajectories and
impact points and areas of spent rocket stages and other components.
"""

import csv
from lxml import etree
from pykml.factory import KML_ElementMaker as KML # libraries for KML exports
# import simplekml # import KML  # to match KML style of other flight safety exports
import math # for radius
from functions_def import convert_j2000_to_geographic, calculate_visibility_radius
from pathlib import Path
import os # for file path finding

import math

def save_kml_output(latitudes, longitudes, altitudes, event_name, trajectory_astos_name, times, time_interval):
    """
    Saves the trajectory and visibility radius information to a KML file for visualization in mapping applications.
    
    Args:
        latitudes (list): List of latitudes along the trajectory.
        longitudes (list): List of longitudes along the trajectory.
        altitudes (list): List of altitudes along the trajectory.
        event_name (str): Name of the event (used for output file naming).
        trajectory_astos_name (str): Name for the output folder.
        times (list): List of time values in seconds.
        time_interval (int): Time interval in seconds to plot visibility circles.
    """
    # Ensure the 'kml_files' directory exists
    output_folder = Path(f"kml_files_{trajectory_astos_name}")
    output_folder.mkdir(parents=True, exist_ok=True)  # Create the folder if it doesn't exist

    altitudes_relative_to_ground = []  # To store the calculated relative altitudes
    for alt in altitudes:
        altitudes_relative_to_ground.append(alt)  # Altitude is relative to ground

    # Create KML document structure
    kml_doc = KML.kml(
        KML.Document(
            KML.name(f"Trajectory {event_name}"),
            
            # Create Style for LineString
            KML.Style(
                KML.LineStyle(
                    KML.color("ff0000ff"),  # Line color (red)
                    KML.width(5)            # Line width
                ),
                id="19050"  # Style ID
            ),
            
            # Add Separation Point Placemark
            KML.Placemark(
                KML.name(f"Separation Point {event_name}"),
                KML.Point(
                    KML.altitudeMode("relativeToGround"),  # Set altitude mode
                    KML.coordinates(f"{longitudes[0]},{latitudes[0]},{altitudes_relative_to_ground[0]}")
                ),
                KML.Style(
                    KML.IconStyle(
                        KML.Icon(
                            KML.href("http://maps.google.com/mapfiles/kml/shapes/placemark_square.png")
                        )
                    )
                )
            ),
            
            # Trajectory LineString Placemark
            KML.Placemark(
                KML.name(f"Trajectory {event_name}"),
                KML.LineString(
                    KML.extrude(1),  # Enable extrusion
                    KML.altitudeMode("relativeToGround"),  # Set altitude mode
                    KML.coordinates(
                        " ".join(f"{lon},{lat},{alt}" for lon, lat, alt in zip(longitudes, latitudes, altitudes_relative_to_ground))
                    )
                ),
                KML.styleUrl("#19050")  # Link to the LineStyle defined above
            ),
        )
    )

    # Add visibility circles at the specified interval
    target_time = 0  # Start at time 0
    threshold = 1.0  # Define an acceptable tolerance (e.g., 1 second)
    next_target_time = time_interval  # Initialize next target time
    for i in range(len(times)):
        # if times[i] > 300: # this is to avoid circles after 300s into flight. Without this, many circles of radius=0 are plotted
        #     break
        # if times[i] < 100:
        #     next_target_time = time_interval/2
        # else:
        #     next_target_time = time_interval
        if abs(times[i] - target_time) < threshold:  # If the time is close to the target time
            altitude = altitudes_relative_to_ground[i]
            visibility_radius = calculate_visibility_radius(altitude)  # Get the visibility radius
            # print(f"Plotting visibility circle at t={times[i]} with radius r={visibility_radius} at h={altitudes_relative_to_ground[i]}") # debugging
            target_time = next_target_time  # Update to the next target interval
            next_target_time += time_interval  # Increment the target time for the next interval

            # Add a circle representing visibility radius
            kml_doc.Document.append(
                KML.Placemark(
                    KML.name(f"Visibility Radius at t={times[i]}s"),
                    KML.Style(
                        KML.LineStyle(
                            KML.color("ff007fff"),  # Outline color (ff00ff00 green, ff007fff orange, 80ffff00  yellow at 50% opacity)
                            KML.width(1)
                        ),
                        KML.PolygonStyle(
                            KML.color("80ffff00"),  # Fill color (7f00ff00 green, 00ffffff no fill, 80ffff00  yellow at 50% opacity)
                            # etree.Element("fill").text = "0",  # Insert <fill>0</fill> # Enable/disable fill
                            KML.outline(True)       # Enable/disable outline
                        )
                    ),
                    KML.Polygon(
                        KML.outerBoundaryIs(
                            KML.LinearRing(
                                KML.coordinates(
                                    " ".join(
                                        f"{longitudes[i] + (visibility_radius / 111320) * math.cos(math.radians(angle)) / math.cos(math.radians(latitudes[i]))},{latitudes[i] + (visibility_radius / 111320) * math.sin(math.radians(angle))},0"
                                        for angle in range(0, 360, 10)  # 36 points to make a circle
                                    )
                                )
                            )
                        )
                    )
                )
            )

            # Add a marker at the center of the circle with the time in seconds
            kml_doc.Document.append(
                KML.Placemark(
                    KML.name(f"{times[i]:.1f}s"),  # Format the time to one decimal place
                    KML.Point(
                        KML.altitudeMode("relativeToGround"),  # Set altitude mode
                        KML.coordinates(f"{longitudes[i]},{latitudes[i]},{altitudes[i]}")  # Center coordinates and altitude
                    ),
                    KML.Style(
                        KML.IconStyle(
                            KML.Icon(
                                KML.href("http://maps.google.com/mapfiles/kml/shapes/placemark_square.png")  # Optional: You can change this to a different icon if needed
                            )
                        )
                    )
                )
            )

    # Save to KML file in the 'kml_files' directory
    output_file = output_folder / f"kml_trajectory_visibility_{event_name}.kml"
    with open(output_file, "wb") as kml_file:
        kml_file.write(etree.tostring(kml_doc, pretty_print=True, xml_declaration=True, encoding='UTF-8'))

    print(f'KML file of {event_name} exported to {output_file}')

def read_and_convert_visibility(csv_filename, event_name, trajectory_astos_name, time_interval=30):
    """
    Propagates the trajectory from a CSV file containing state vectors, converts the coordinates
    from J2000 to geographic coordinates, and saves the results in a KML format.
    Plots visibility circles in the first phase of the trajectory, to show where observers from ground can see the launch.

    Args:
        csv_filename (str): The name of the CSV file containing the state vectors (time, x, y, z).
        event_name (str): The name of the event associated with the trajectory, used for naming outputs.
        trajectory_astos_name (str): Name to be used for the folder where the CSV file is saved.
        time_interval (int): Time interval in seconds to plot visibility circles.

    Returns:
        None: The function writes the KML output to a file after processing the trajectory data.
    """
    latitudes = []
    longitudes = []
    altitudes = []
    times = []

    csv_filename = os.path.join(f'csv_files_{trajectory_astos_name}', csv_filename)  # Ensure correct file path
    with open(csv_filename, mode='r') as csv_file:
        csv_reader = csv.reader(csv_file)
        header = next(csv_reader)  # Skip the header

        for row in csv_reader:
            # if len(times) > 15:  # Limit to first 10 rows for testing
            #     print('porcodio')
            #     break
            time = float(row[0])  # Time in seconds
            x = float(row[1])      # X position in km
            y = float(row[2])      # Y position in km
            z = float(row[3])      # Z position in km

            lat, lon, alt = convert_j2000_to_geographic(x, y, z, time)
            times.append(time)

            # hard fix to move starting point to launch pad
            # launch long and lat with 00tt reference epoch: -1.5486284922357072,60.67360076667758
            # actual launch pad coords 60deg 49' 07'' (N), 0deg 46'27'' (W), converted to 60.81861111111111, -0.7741666666666667
            lon += (-0.7742611111111112 - 178.00073254712802) # difference real wrt computed
            lat += (60.81879722222222 - 60.837675302194945) # difference real wrt computed

            latitudes.append(lat)
            longitudes.append(lon)
            altitudes.append(alt)

    save_kml_output(latitudes, longitudes, altitudes, event_name, trajectory_astos_name, times, time_interval)
    print(f'KML file of {event_name} exported')


# Example usage with a different time interval (e.g., 60 seconds)
read_and_convert_visibility("state_vector_full_trajectory.csv", "full_trajectory", "traiettoria", time_interval=30)
